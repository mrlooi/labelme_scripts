...
def updateETTree(ET_obj, output_file, poly_pts, poly_cls, h, w):

	assert(len(poly_cls) == len(poly_pts)) 
...
if __name__ == '__main__':
	...
	for ix, im_path in enumerate(img_paths):
	 ...
	 #    # annot file
		annot_file = osp.join(annot_dir, im_path + ".xml")
		...
		for e in element_objects:
			...
			e_pts_cls.append(e.find('name').text)
		# resize
		...
		updateETTree(et, annot_file_new, resized_pts, e_pts_cls, TARGET_HEIGHT, TARGET_WIDTH)
